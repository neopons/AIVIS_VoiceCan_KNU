...
def model_to_tflite(sess,
  ...
  # Avoid streaming mode and do not convert if non_stream
  if mode == modes.Modes.NON_STREAM_INFERENCE:
    if save_model_path:
      save_model_summary(model_non_stream, save_model_path)

    # convert Keras inference model to tflite inference model
    converter = tf1.lite.TFLiteConverter.from_session(
        sess, model_non_stream.inputs, model_non_stream.outputs)
