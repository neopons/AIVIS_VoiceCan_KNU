...
def which_set(filename, validation_list, testing_list):
  ...
  """
  pathname = filename.split('/')[-2]+'/'+filename.split('/')[-1]+"\n"
  if pathname in validation_list:
    result = 'validation'
  elif pathname in testing_list:
    result = 'testing'
  else:
    result = 'training'
  return result
...
class AudioProcessor(object):
    ...
    if flags.data_dir:
      ...
      if flags.split_data:
        ...
        self.prepare_data_index(flags.silence_percentage,
                                flags.unknown_percentage, wanted_words, flags.validation_list_dir,
                                flags.testing_list_dir, flags.split_data, self.data_dir)
      else:
  ...
  def maybe_download_and_extract_dataset(self, data_url, dest_directory):
    ...
    if not os.path.exists(dest_directory):
      raise Exception('No dirs ' + dest_directory)


  def prepare_data_index(self, silence_percentage, unknown_percentage,
                         wanted_words, validation_list_dir,
                         testing_list_dir, split_data, data_dir):
    ...
    fv = open(data_dir+'/'+validation_list_dir, "r")
    validation_list = fv.readlines()
    ft = open(data_dir+'/'+testing_list_dir, "r")
    testing_list = ft.readlines()
    ...
    for wav_path in gfile.Glob(search_path):
      ...
      if word == BACKGROUND_NOISE_DIR_NAME:
        continue
      all_words[word] = True
      set_index = which_set(wav_path, validation_list, testing_list)
